...
When `dropdown = true` and you have 15 or fewer articles, they appear directly in a dropdown. With more articles, a link to the section index is shown instead.

### Articles Index Page

Declare the articles directory as a section and QuartoDocBuilder will create the section index automatically if it is missing:
...
config = QuartoConfig(
    ...
    sections = [
        SectionConfig(
            title = "Articles",
            dir = "articles",
            desc = "Long-form guides and tutorials."
        )
    ]
)
...
This creates `docs/articles.qmd` with a Quarto listing of all `.qmd` files inside `docs/articles/`.
